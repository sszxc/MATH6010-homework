...
def draw_star_expansion_graph(graph):
    """
    根据名称区分节点类型"node""edge", 绘制超图
    """
    fig, ax = plt.subplots()
...
if __name__ == '__main__':
    ...
    for i in range(edge_num):                       # 添加超边
        ...
        for j in random.sample(range(node_num), 3): # 连接超边和顶点
            star_expansion_graph.add_edge('edge'+str(i),
                                    'node'+str(j))

    draw_star_expansion_graph(star_expansion_graph)
    plt.show()